...
import fondoImg from "../../assets/pokeball.jpg";
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 h-screen w-full">
      <div className="bg-white flex flex-col justify-center">
        <form
          onSubmit={onSubmit}
          className="max-w-[500px] w-full mx-auto bg-gray-900 p-8 px-8
            rounded-3xl shadow-xl shadow-emerald-500/50"
        >
          <h2 className="text-4xl text-emerald-300 font-bold text-center">
            LOGIN
          </h2>
          <div className="flex flex-col text-white py-2">
            <label> User Name </label>
            <input
              className="rounded-md bg-gray-300 mt-2 p-2 focus:bg-gray-400
          ...
          <div className="flex flex-col text-white py-2">
            ...
            <input
              className="rounded-md bg-gray-300 mt-2 p-2 focus:bg-gray-400 
              focus:outline.none text-black required:"
              type={shown ? "text" : "password"}
              {...register("password", { required: true })}
            />
            <FaRegEye
              className=" text-black absolute w-10 ml-96 mt-11"
              onClick={ switchShown }
            />
          </div>
          <button
            onClick={ loader }
            className="rounded-md w-full my-8 py-2 bg-emerald-500
              hover:bg-emerald-300/50 text-white"
          >
            { loading ?
            <svg
              aria-hidden="true"
      ...
      <div className="hidden sm:block">
        <img className="w-full h-full object-cover" src={ fondoImg } alt="" />
      </div>
    </div>
  );
};
